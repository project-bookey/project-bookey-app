...
테마는 원래 기기(OS) 다크 모드 설정을 따라갔지만, 2026-09-01 후속 결정으로 리디자인 검수
기간 동안 **다크 고정** 상태다(토큰 스펙 "후속 결정" 절 참조 — 시스템 라이트에서 리디자인
화면만 밝아지는 톤 불일치 때문). 이 작업은 그 다크 고정을 **앱 내 수동 테마 전환**으로
대체한다. 토큰 설계 때 예고한 확장 지점("수동 전환이 필요해지면 `useTheme()` 내부만
바꾼다")을 그대로 사용한다 — 호출부는 변경하지 않는다.

## 결정 요약

| 항목 | 결정 | 근거 |
|---|---|---|
| 모드 선택지 | 시스템 / 라이트 / 다크 3택, 기본 '다크' | 기본 동작은 현재(다크 고정)와 동일하게 보존. 레거시 화면이 남은 동안 시스템/라이트는 톤 불일치를 감수하는 명시적 선택(opt-in)으로만 |
...
- 초기값·폴백 모두 `'dark'` — 아무것도 선택하지 않은 사용자는 현재의 다크 고정과 동일하게 동작.
...
- 현재의 다크 고정(`mode = 'dark'` 하드코딩)을 위 해석 로직으로 대체한다 —
  `useColorScheme` import도 복원.
- 반환 시그니처 `{ mode, colors, cardShadow }` 불변 — 기존 호출부(홈·서재·알림·탭바·루트
  레이아웃) 변경 zero.
- '시스템' 선택 시 OS 설정 변경은 `useColorScheme()` 리렌더로 실시간 반영된다.
- **후속 메모**: 레거시 화면의 OTT 리디자인이 모두 끝나 톤 불일치가 사라지면
  기본값을 `'dark'` → `'system'`으로 되돌리는 것을 검토한다.
