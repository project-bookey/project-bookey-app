...
- 담기 실패 시(중복·네트워크 등) 해당 행에 `warn` 색 캡션 "담지 못했어요 · 다시 시도"를
  표시하고 ①(담기 버튼)로 복귀한다 — 이 저장소에는 뮤테이션 오류를 자동 노출하는 공통
  흐름이 없으므로(각 화면이 직접 처리하는 관례) 행 단위로 직접 표시한다.
- 검색어가 바뀌면 열려 있던 담기 칩(`openAddId`)과 실패 표시는 초기화한다.
