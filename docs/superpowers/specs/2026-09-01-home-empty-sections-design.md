...
## 후속 결정 (2026-09-01, 같은 날)

온보딩 히어로(높이 360 "첫 책을 찾아보세요" + 책 찾기 버튼)가 과하다는 피드백 →
검색 진입점을 **홈 최상단 상시 가짜 검색 바**로 교체한다 (시각 비교로 선택).

- **검색 바** — 홈 스크롤 맨 위(배너 위)에 항상 표시. hairline 테두리(`colors.lineStrong`),
  `radius.md`, 안에 `⌕ 책 제목·저자 검색`(`colors.textMuted`). 누르면 `/search`.
  `accessibilityRole="button"` + "책 검색" 라벨. 책이 있는 사용자에게도 홈 검색 진입점이 생긴다.
- **히어로** — 온보딩 빈 상태 분기와 `onSearch` prop을 제거하고, 읽는 중 기록이 없으면
  렌더하지 않는다. 로딩 스켈레톤은 유지.
- 빈 "읽는 중"·"읽고 싶은" 행의 + 타일은 그대로 둔다 (행 단위 추가 진입점).
