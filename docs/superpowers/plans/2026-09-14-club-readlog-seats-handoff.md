...
`app/notifications.tsx` 는 탭하면 읽음 처리만 하고 이동하지 않는다. 주간 알림 `CLUB_WEEKLY_LOG` 의 payload 는 `{ clubId, weekOf }` —
`/club/[id]/log/week?weekOf=` 로 보내면 된다. 다른 모임 알림(`CLUB_NEW_POST`·`CLUB_ENDED` 등)도 같이 정리할 것.

### 2-5. 로컬 서버 기동 오류 (팀원 커밋, 미수정)
...
남의 코드라 고치지 않았다 — 담당자에게 공유하거나 합의 후 수정. 그 전까지 로컬은 아래 우회로로 띄운다.
...
- 결산의 `topDiscussant`(토론왕)가 `countByClubIdAndUserIdAndStatus` 로 세서 **LOG 조각도 글 수에 들어간다** — 토론 글만 세도록 고칠지 결정 필요
...
MAIL_ENABLED=true SPRING_MAIL_HOST=localhost npm run dev  # 2-5 우회 — 메일은 실제로 보내지 않는다
