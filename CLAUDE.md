...
- **Navigation**: the Stack sets `headerShown: false` globally — there is no native header anywhere. Each screen draws its own chrome: the four sections (서가 `/home` · 탐색 `/search` · 광장 `/plaza` · 나 `/profile`) render `SectionNav` at the top, and sub-screens render `SubHeader`. Screens still register a Korean `title`, which the web build uses as the browser tab title. Moving *between sections* uses `router.navigate` (or `replace` inside `SectionNav`) so the stack never stacks duplicate section entries; pushing to a sub-route (`/book/[id]`, `/library`, `/clubs`, `/timer`, ...) uses `router.push`.
- **Path alias**: `@/*` → `src/*`.
